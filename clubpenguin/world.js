...
xt_handlers = {
	's': {
		...
		'j#jr': {event: 'handleJoinRoom', throttle: true},
		'j#jp': {event: 'handleJoinPlayer', throttle: true},
		...
		'u#h': {event: 'sendHeartbeat', throttle: false},
		'u#se': {event: 'sendEmote', throttle: true},
		'u#ss': {event: 'sendSafeChat', throttle: true},
		'u#sg': {event: 'sendTourMessage', throttle: true},
		'u#sp': {event: 'sendPosition', throttle: true},
		'u#sb': {event: 'sendSnowball', throttle: true},
		'u#sf': {event: 'sendFrame', throttle: true},
		'u#sa': {event: 'sendAction', throttle: true},
		'u#sl': {event: 'sendLine', throttle: true},
		'u#sj': {event: 'sendJoke', throttle: true},
		'u#glr': {event: 'sendLastRevision', throttle: false},
		'u#gp': {event: 'getPlayer', throttle: true},
		'u#tp': {event: 'sendTeleport', throttle: true, enabled: false},
		
		's#upc': {event: 'updatePlayerClothing', throttle: false},
		...
		'p#pt': {event: 'sendPuffleTreat', throttle: false},
		'p#pf': {event: 'sendPuffleFeed', throttle: false},
		'p#pip': {event: 'sendPuffleInitInteractionPlay', throttle: false},
		'p#pir': {event: 'sendPuffleInitInteractionRest', throttle: false},
		'p#ir': {event: 'sendPuffleInteractionRest', throttle: false},
		'p#ip': {event: 'sendPuffleInteractionPlay', throttle: false},
		'p#pr': {event: 'sendPuffleRest', throttle: false},
		'p#pp': {event: 'sendPufflePlay', throttle: false},
		'p#pb': {event: 'sendPuffleBath', throttle: false},
		'p#ps': {event: 'sendPuffleFrame', throttle: false},
		'p#pw': {event: 'sendPuffleWalk', throttle: false},
		'p#pm': {event: 'sendPuffleMove', throttle: false},
...
function start(showLog) {
	...
	for(var index in dependencies['world']) {
		...
		try {
			require(dependencyDetails.join(''))();
		} catch(error) {
...
function handle(penguin, data) {
	...
	if(handler in xt_handlers[type] && events_array.indexOf(handler) >= 0) {
		if(penguin.joined == true && penguin.room !== undefined) {
			...
			if(xt_handlers[type][handler].throttle == true && !penguin.room.is_game) {
