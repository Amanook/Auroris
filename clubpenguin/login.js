...
function finishLogin(penguin, id, banned, buddies, moderator, active, loginIPs) {
	...
	if(banned !== '0') {
		...
		if(banned_date > now_date) {
			...
			return penguin.send('e', -1, 601, hours);
		}
	}
...
function handle_world(penguin, nick, passwd) {
	...
	penguin.database.player_exists(nick, function(result) {
		...
		penguin.database.get_columns_name(nick, ['LoginKey', 'ID', 'Username', 'Moderator'], function(row) {
			var id = row['ID'];
			var username = row['Username'];
			var moderator = row['Moderator'];
			var db_password = row['LoginKey'];
			var world_hash = crypto.world_key(db_password + penguin.key) + db_password;
