...
function removePenguin(penguin) {
	try {
		if(penguins.indexOf(penguin) >= 0) {
			if(penguin.joined) {
				...
				if(typeof(penguin.buddylist) == 'object') {
					for(var index in Object.keys(penguin.buddylist)) {
						var buddyId = Object.keys(penguin.buddylist)[index];
			...
			var index = penguins.indexOf(penguin);
		...
		fs.appendFile('socketerr.txt', ("socket err --> " + erro + "\n\n"), function(error_str) {
			if(error_str) {
